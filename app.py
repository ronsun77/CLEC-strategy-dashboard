...
if not df_comp.empty:
    ...
    with st.expander("📖 點擊查看【再平衡心法】與【量化指標】白話文說明", expanded=False):
        ...
        * **年度定時 (+15%/-10%觸發)**：每年底檢查一次。以設定權重為基準（例：QLD 50%），當 QLD 在「總資產的佔比」膨脹超過 65% (50+15)，或縮水低於 40% (50-10) 時，才執行再平衡洗牌；若未達標，則當年度不動作，節省摩擦成本。
        * **閾值平衡 (±50%)**：每日監控。不看總資產佔比，只看「槓桿資產本身的價值」。當 QLD 自身價值比上次再平衡時上漲了 50%（爆發）或下跌了 50%（腰斬）時，立刻在盤中觸發再平衡。
